...
// Přidává event listener na cookie element, který se spustí při kliknutí
document.getElementById('cookie').addEventListener('click', (event) => {
    // Zvýší skóre o hodnotu klikIncrement (počet cookies za jeden klik)
    skore += klikIncrement;
    // Aktualizuje text skóre na obrazovce
    document.getElementById('skore').textContent = 'cookies: ' + skore;
    // Aktualizuje stav tlačítek pro nákup vylepšení
    aktualizujTlacitka();

    // Vytvoří nový div element pro zobrazení "+X" efektu
    const plusJedna = document.createElement('div');
    plusJedna.className = 'plus-jedna';
    plusJedna.textContent = '+' + klikIncrement;

    // Nastaví offset pro pozicování efektu vzhledem k místu kliknutí
    const offsetX = -18;
    const offsetY = -30;
    // Nastaví pozici efektu podle souřadnic kliknutí
    plusJedna.style.left = (event.clientX + offsetX) + 'px';
    plusJedna.style.top = (event.clientY + offsetY) + 'px';
    document.body.appendChild(plusJedna);

    // Nastaví čas, po kterém se efekt smaže 
    let pockej = 1000;

    // Po uplynutí času odstraní 1
    setTimeout(() => {
        plusJedna.remove();
    }, pockej);
});
